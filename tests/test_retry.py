...
from cmdrdata_anthropic.retry import (
    RetryPolicy, RetryConfig, CircuitBreaker, CircuitBreakerState,
    retry_with_backoff, DEFAULT_RETRY_CONFIG, AGGRESSIVE_RETRY_CONFIG,
    CONSERVATIVE_RETRY_CONFIG, DEFAULT_CIRCUIT_BREAKER
)
from cmdrdata_anthropic.exceptions import (
    RetryExhaustedError, CircuitBreakerError, NetworkError, TimeoutError
)
...
class TestRetryDecorator:
    ...
    async def test_retry_decorator_with_circuit_breaker(self):
        ...
        @retry_with_backoff(
            RetryConfig(max_attempts=5, initial_delay=0.01),
            circuit_breaker=cb
        )
        async def test_function():
            nonlocal call_count
