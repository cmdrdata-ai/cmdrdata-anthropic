...
from cmdrdata_anthropic.performance import (
    LRUCache, CacheEntry, ConnectionPool, RequestBatcher, RateLimiter,
    PerformanceMonitor, cached, timed, PerformanceContext,
    get_cache_stats, get_performance_stats, clear_cache, configure_performance
)
...
class TestRequestBatcher:
    ...
    async def test_request_batcher_basic(self):
        ...
        tasks = []
        for i in range(2):
            task = asyncio.create_task(batcher.add_request(f"request_{i}", mock_processor))
            tasks.append(task)
    ...
    async def test_request_batcher_error_handling(self):
        ...
        tasks = []
        for i in range(2):
            task = asyncio.create_task(batcher.add_request(f"request_{i}", failing_processor))
            tasks.append(task)
